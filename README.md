...
| Reliability    | Для менеджмента задач: требование от клиентов на удобную и стабильную работу                  |                                                          |
...
| Reliability    | Требование от клиентов на удобную и стабильную работу                                         |                                 |
...
В целом у сервиса требования как у основной части, добавляется только требование к консистентности из-за финансового
комплаенса и необходимо иметь возможность расширить способы оплат для клиентов. 
...
| Agility        | Требование низкого TTM                                                                        |                               |
| Evolvability   | Система работает в инновационном секторе и проверка гипотез с быстрой реакцией как требование |                               |
| Security       | В системе есть разграничение ролей                                                            | Клиент, Воркер, Бухгалтер     |
| Consistency    | Клиент и воркер имеют доступ к своим данным, данные не теряются и соответствуют комплаенсу    |                               |
| Modifiability  | Для подготовки инвойса клиентского и проведения платежа: необходимо расширять способы оплаты  |                               |
| Modularity     | Для подготовки инвойса клиентского и проведения платежа: необходимо расширять способы оплаты  |                               |
...
~Расчет и подготовка списания средств с клиента~
...
| Agility        | Требование низкого TTM                                                                        |                               |
| Evolvability   | Система работает в инновационном секторе и проверка гипотез с быстрой реакцией как требование |                               |
| Security       | В системе есть разграничение ролей                                                            | Клиент, Бухгалтер             |
| Consistency    | Клиент и воркер имеют доступ к своим данным, данные не теряются и соответствуют комплаенсу    |                               |
| Modifiability  | Необходимо расширять способы оплаты                                                           |                               |
| Modularity     | Необходимо расширять способы оплаты                                                           |                               |

~Расчет и подготовка начисления средств воркеру~
...
| Agility        | Требование низкого TTM                                                                        |                               |
| Evolvability   | Система работает в инновационном секторе и проверка гипотез с быстрой реакцией как требование |                               |
| Security       | В системе есть разграничение ролей                                                            | Воркер, Бухгалтер             |
| Consistency    | Клиент и воркер имеют доступ к своим данным, данные не теряются и соответствуют комплаенсу    |                               |

~Проведение платежа~
Будем покупать внешний payment provider, должен удовлетворять комплаенсу и расширяться способами оплаты
...
| Modifiability  | Необходимо расширять способы оплаты                                                        |             |
| Modularity     | Необходимо расширять способы оплаты                                                        |             |
| Security       | Внешняя система принимает от нас личные и финансовые данные                                |             |
...
| Agility        | Требование низкого TTM                                                                        |                               |
| Evolvability   | Система работает в инновационном секторе и проверка гипотез с быстрой реакцией как требование |                               |
| Security       | В системе есть разграничение ролей                                                            | Менеджер?                     |
| Consistency    | В сервисе происходит работа с финансами => по комплаемсу должна быть сохранность данных       |                               |

**ADR:** To be filled

## Характеристики не менявшихся сервисов
...
| Agility               | Требование низкого TTM                                                                        |                               |
| Evolvability          | Система работает в инновационном секторе и проверка гипотез с быстрой реакцией как требование |                               |
| Security              | В системе есть разграничение ролей                                                            | Воркер, работник склада       |
| Consistency           | Склад должен работать в соответствии с текущим описанием типа задачи и задания                |                               |
| High model complexity | Работает со сложными моделями заказа и воркера, сопоставляет характеристики                   |                               |
